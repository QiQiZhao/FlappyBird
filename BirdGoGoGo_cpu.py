...
def save_pool():
    for xi in range(total_models):
        torch.save(current_pool[xi].state_dict(), "Current_Model_Pool//model_new" + str(xi) + ".pytorch")
    print("Saved current pool!")

def model_crossover(model_idx1, model_idx2):
    global current_pool
    weights1 = current_pool[model_idx2].fc1.weight.clone().detach()
    weights2 = current_pool[model_idx1].fc1.weight.clone().detach()
...
def predict_action(height, dist, pipe_height, model_num):
    ...
    neural_input = torch.tensor([height, dist, pipe_height],dtype=torch.float32)
    output_prob = current_pool[model_num](neural_input)
...
if load_saved_pool:
    for i in range(total_models):
        current_pool[i].load_state_dict(torch.load("Current_Model_Pool//model_new"+str(i)+".pytorch",map_location=torch.device('cpu')))


for i in range(total_models):
    print(current_pool[i].fc1.weight)
    ...
    for select in range(int(total_models/2)):
        ...
        for idxx in range(total_models):
            if fitness[idxx] >= parent2:
                idx2 = idxx
                break
        updated_weights1,updated_weights2 = model_crossover(idx1, idx2)
        new_weights.append(updated_weights1)
        new_weights.append(updated_weights2)
    for select in range(len(new_weights)):
        fitness[select] = -100
        current_pool[select].fc1.weight=torch.nn.Parameter(new_weights[select])
