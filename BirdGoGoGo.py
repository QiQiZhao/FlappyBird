...
def save_pool():
    for xi in range(total_models):
        torch.save(current_pool[xi].state_dict(), "Current_Model_Pool//model_new" + str(xi) + ".keras")
...
def predict_action(height, dist, pipe_height, model_num):
    ...
    neural_input = torch.tensor([height, dist, pipe_height],dtype=torch.float32).cuda()
...
if load_saved_pool:
    for i in range(total_models):
        current_pool[i].load_state_dict(torch.load("Current_Model_Pool//model_new"+str(i)+".keras"))
...
def showGameOverScreen(crashInfo):
    ...
    for select in range(len(new_weights)):
        fitness[select] = -100
        current_pool[select].fc1.weight=torch.nn.Parameter(torch.tensor(new_weights[select]))
